    ...
    >
      <div className="bg-card rounded-2xl border border-border/50 shadow-toss hover:shadow-toss-md transition-toss active:scale-[0.99] overflow-hidden">
        ...
        <div className="flex items-center gap-3 px-4 py-3" onClick={(e) => e.stopPropagation()}>
          <Link href={`/profile/${tag.reporterId}`} className="w-9 h-9 rounded-full bg-gradient-to-br from-toss-orange/20 to-amber-400/20 flex items-center justify-center shrink-0 ring-2 ring-border/50 hover:ring-primary/30 transition-all">
            <span className="text-[12px] font-bold text-toss-orange">{tag.reporterName.charAt(0)}</span>
          </Link>
          <div className="flex-1 min-w-0">
            <div className="flex items-center gap-1.5">
              ...
              <RankBadge rank={tag.reporterRank} />
              <TitleBadge title={tag.reporterTitle} />
            </div>
            <p className="text-[11px] text-toss-gray-400">{formatRelativeTime(tag.createdAt)}</p>
          </div>
          <span className={`shrink-0 inline-flex items-center gap-1 px-2 py-0.5 rounded-md text-[11px] font-semibold ${info.bg} ${info.color}`}>
        ...
        <div className="px-4 pb-3">
          <div className="flex items-center gap-2 mb-2">
            <h3 className="text-[16px] font-bold text-foreground truncate">{tag.nickname}</h3>
          ...
          <p className="text-[13px] text-toss-gray-700 dark:text-toss-gray-300 line-clamp-2 leading-relaxed">{tag.description}</p>
        </div>
        ...
        <div className="px-4 py-2.5 border-t border-border/30 flex items-center justify-between text-[11px]">
          <div className="flex items-center gap-3">
            <span className="flex items-center gap-1 text-toss-green font-medium">
          ...
          <span className="text-toss-gray-400 shrink-0 ml-3">{formatDateTime(tag.createdAt)}</span>
        </div>
      </div>
    </div>
  );
